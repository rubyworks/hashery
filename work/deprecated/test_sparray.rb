...
# SparseArray is tested by comparison to the standard Array.
...
class TC_SparseArray < Test::Unit::TestCase
  ...
  def test_coverage
    assert_equal([], Array.instance_methods - SparseArray.instance_methods)
  end
  ...
  def test_get_operations
    a, ha = aha [0,1,2,3,4,5,6,7,8]
    assert_equal(a[1], ha[1])
    assert_equal(a[-1], ha[-1])
    assert_equal(a[-3], ha[-3])
  end

  def test_get_operations_size
    a, ha = aha [0,1,2,3,4,5,6,7,8]
    assert_equal(a[0,2], ha[0,2].to_a)
    assert_equal(a[1,1], ha[1,1].to_a)
    assert_equal(a[-3,2], ha[-3,2].to_a)
    assert_equal(a[-4,3], ha[-4,3].to_a)
  end

  def test_get_operations_range
    a, ha = aha [0,1,2,3,4,5,6,7,8]
    assert_equal(a[0..2], ha[0..2].to_a)
    assert_equal(a[2..-1], ha[2..-1].to_a)
  end
  ...
  def test_insert
    a, ha = aha %w{ a b c d }
    assert_equal(a.insert(2, 99), ha.insert(2,99).to_a)              #=> ["a", "b", 99, "c", "d"]
    a, ha = aha %w{ a b c d }
    assert_equal(a.insert(-2, 1, 2, 3), ha.insert(-2, 1, 2, 3).to_a) #=> ["a", "b", 99, "c", 1, 2, 3, "d"]
  end
  def test_join
    a, ha = aha [2,3,4]
